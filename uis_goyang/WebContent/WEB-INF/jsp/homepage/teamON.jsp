<%@ page language="java" contentType="text/html; charset=EUC-KR"
    pageEncoding="EUC-KR"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=EUC-KR">
<title>homepage</title>
    <link href="css/menu.css" rel="stylesheet" />
</head>
<body>
        <!-- Navigation-->
        <nav class="navbar navbar-expand-lg navbar-dark fixed-top" id="mainNav">
            <div class="container">
                <a class="navbar-brand js-scroll-trigger" href="loginON.do"><img src="assets/img/homepageLOGO.png" alt="" /></a>
                <button class="navbar-toggler navbar-toggler-right" type="button" data-toggle="collapse" data-target="#navbarResponsive" aria-controls="navbarResponsive" aria-expanded="false" aria-label="Toggle navigation">
                    Menu
                    <i class="fas fa-bars ml-1"></i>
                </button>
                <div class="collapse navbar-collapse" id="navbarResponsive">
                    <ul class="navbar-nav text-uppercase ml-auto">
                        <li class="nav-item"><a class="nav-link js-scroll-trigger" href="servicesON.do">Services</a></li>
                        <li class="nav-item"><a class="nav-link js-scroll-trigger" href="portfolioON.do">Portfolio</a></li>
                        <li class="nav-item"><a class="nav-link js-scroll-trigger" href="aboutON.do">About</a></li>
                        <li class="nav-item"><a class="nav-link js-scroll-trigger" href="#">Team</a></li>
                        <li class="nav-item"><a class="nav-link js-scroll-trigger" href="logoutAction.do">Logout</a></li>
                        <ul class="nav-item">
                        	<li class="nav-link js-scroll-trigger"><a href="kakaoMap.do">search</a></li>
                        </ul>
                    </ul>
                </div>
            </div>
        </nav>
        <div class="box">
        <h2>[ (진행중) ]</h2>
        <img src="images/man.jpg" id="man">
        <p>- 중앙대학교 건축학과 졸업<br>
		- 녹색건축기술 분야에서 2007년부터 현재까지 14년간 근무<br>
		- 국내건축물 인증 및 미국의 LEED, 영국의 BREEAM등 건물인증 시스템 전문가<br>
		- 컴퓨터 시뮬레이션 (건축물 에너지성능, 일조, 기류환경 등) 주거환경성능 분석 전문가<br>
		- 미국 EVO의 CMVP (Certified Measurement and Verification Professional) 자격<br>
		- 건물에너지관리시스템(BEMS)의 개발에 참여, IT기술과 건축기술의 접목과정에 대한 경험<br>
        </p>
        </div>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
        <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.5.3/dist/js/bootstrap.bundle.min.js"></script>
        <!-- Third party plugin JS-->
        <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-easing/1.4.1/jquery.easing.min.js"></script>
        <!-- Contact form JS-->
        <script src="assets/mail/jqBootstrapValidation.js"></script>
        <script src="assets/mail/contact_me.js"></script>
        <!-- Core theme JS-->
        <script src="js/scripts.js"></script>
        
    
</body>
</html>